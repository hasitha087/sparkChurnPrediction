...
object churnRF {
  ...
  val conf = new SparkConf().setAppName("RF-Churn-Prediction")
  val sc = new org.apache.spark.SparkContext(conf)
  val sqlContext = new org.apache.spark.sql.SQLContext(sc)
  val filePath = new File("").getAbsolutePath
  val config = ConfigFactory.parseFile(new File(filePath + "/../conf/churnRF.conf"))
  
  def main(args: Array[String]): Unit={
      
      val data1 = sqlContext.read.parquet("/user/hive/warehouse/db1.db/input_table")  //Read from hive table
      data1.registerTempTable("churn_train_input")
      val query = s"select feature1,feature2,feature3,feature4,feature5,feature6, cast(output as double) as label from churn_train_input" //Edit features from select
      val dataset = sqlContext.sql(query)
	  val col = dataset.select("feature1","feature2","feature3","feature4","feature5","feature6").columns //Edit features columns
      val assembler = vectorAssembler(dataset.na.drop,col)
	  ...
	  val labelIndexer = new StringIndexer().setInputCol("label").setOutputCol("indexedLabel").fit(normalize(assembler.na.drop))
	  ...
	  sc.parallelize(Seq(cvModel), 1).saveAsObjectFile(config.getString("path"))
      println("****************CX CHURN Prediction Model Saved in: " + config.getString("path") + "***********")
